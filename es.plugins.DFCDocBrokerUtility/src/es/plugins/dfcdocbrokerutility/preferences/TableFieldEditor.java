...
public abstract class TableFieldEditor extends FieldEditor {
	...
	protected TableFieldEditor(String name, String labelText,
			String[] columnNames, int[] columnWidths, Composite parent, IWorkbench workbench) {
		init(name, labelText);
		this.columnNames = columnNames;
		this.columnWidths = columnWidths;
	...
	private String[][] getCommands(TableItem[] items){
	...
	protected void doStore() {
		String s = createList(getCommands(table.getItems()));
		if (s != null) {
...
/*		setPresentsDefaultValue(false);
		...
		AddDocBrokerWizard addw=new AddDocBrokerWizard();
		addw.init(wbench, new StructuredSelection(),createList(getCommands(table.getItems())));
		
		WizardDialog dialog = new WizardDialog(parent.getShell(), addw);
	    dialog.create();
	    dialog.open();
